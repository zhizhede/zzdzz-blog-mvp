...
const visibilityOptions: Array<'public' | 'private' | 'draft'> = ['public', 'private', 'draft']
...
function toggleTag(tid: number) {
  const i = form.value.tag_ids.indexOf(tid)
...
onMounted(async () => {
  ...
  if (isEdit) await loadArticle(id)
...
<template>
  <div class="page">
    <div class="page-head">
      <IssueTag prefix="ADMIN" text="EDITOR" :suffix="isEdit ? `EDIT #${id}` : 'NEW'" />
      <h1 class="display title">{{ isEdit ? '编辑文章' : '新文章' }}</h1>
      ...
      <div class="col col-form">
        ...
        <div class="field">
          ...
          <div class="tag-row">
            ...
            <span v-if="!allTags.length" class="muted">暂无标签,去后台 → 标签管理创建</span>
          </div>
        </div>
